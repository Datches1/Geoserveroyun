...
const Game = () => {
  ...
  const [hintsRemaining, setHintsRemaining] = useState(
    difficulty === 'duo' ? 0 : (user?.role === 'premium-player' ? 8 : 4)
  );
    ...
    if (gameOver) {
      const saveScore = async () => {
        ...
        if (difficulty === 'duo') {
          ...
          return; // Exit early, don't save score
        }
        ...
        if (isAuthenticated && user) {
          try {
            ...
            const response = await gameAPI.saveScore(scoreData);
  ...
  }, [gameOver, isAuthenticated, user, difficulty, score, questionCount, correctlyAnsweredQuestions, timeLeft]);
  ...
  const startGame = () => {
    ...
    setHintsRemaining(difficulty === 'duo' ? 0 : (user?.role === 'premium-player' ? 8 : 4));
    setShowHint(false);
  ...
  const handleMapClick = (clickedProvinceName) => {
    ...
    if (isCorrect) {
      if (difficulty === 'duo') {
        ...
        const basePoints = difficulty === 'hard' ? 15 : 10;
        const multiplier = user?.role === 'premium-player' ? 1.25 : 1;
  ...
  const handleHintClick = () => {
    console.log('Hint clicked - User:', user);
    console.log('User role:', user?.role);
    const isAdmin = user?.role === 'admin';
    console.log('Is admin?', isAdmin);
    
    if (!isAdmin && hintsRemaining <= 0) {
          ...
          {difficulty !== 'duo' && (
            <div className="top-right-controls">
              <button 
                ...
                title={user?.role === 'admin' ? 'İpucu (Sınırsız)' : `İpucu (${hintsRemaining} kalan)`}
                disabled={user?.role !== 'admin' && hintsRemaining <= 0}
              >
                <span>💡</span>
                {user?.role !== 'admin' && <span className="hint-counter">{hintsRemaining}</span>}
              </button>
            </div>
          )}
